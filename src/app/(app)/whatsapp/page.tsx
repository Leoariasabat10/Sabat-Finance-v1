...
export default async function Page() {
  ...
  return (
    <div className="animate-fade-up flex flex-col gap-5">
      ...
      {contactos.length === 0 ? (
        <EmptyState icon="✅" title="Nadie por contactar hoy" description="No hay pagos vencidos ni que venzan hoy." />
      ) : (
        <div className="flex flex-col gap-3">
          {contactos.map((c) => (
            <Card key={c.operacionId} className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between">
